...
class HumanoidEnv:
    ...
    def run(self): 
        self.left_gripper_center_id = self.model.site("Left_gripper_center").id
        self.right_gripper_center_id = self.model.site("Right_gripper_center").id       
        self.is_left_relative_control = False
        self.is_right_relative_control = False
        global out
        for i in range(int(self.sim_duration / self.sim_dt)):
            ...
            if out:
                ...
                print(f"Left gripper target position: {mink.SE3.from_mocap_name(self.model, self.data, 'Left_gripper_center_target').wxyz_xyz[4:]}")
                print(f"Right gripper target position: {mink.SE3.from_mocap_name(self.model, self.data, 'Right_gripper_center_target').wxyz_xyz[4:]}")
                self.is_left_relative_control = not self.is_left_relative_control
                self.is_right_relative_control = not self.is_right_relative_control
                out = not out
            ...
            if i > 1000 and self.mink_init:
                T_left = mink.SE3.from_mocap_name(self.model, self.data, "Left_gripper_center_target")
                T_right = mink.SE3.from_mocap_name(self.model, self.data, "Right_gripper_center_target")
                ...
                if not self.is_left_relative_control:
                    T_left_target_position_reduced, T_left_target_wxyz_reduced = world_frame_to_reduced_world_frame(
                            self.model, self.data, T_left_target_position, T_left_target_wxyz
                        )
                    ...
                    T_left_target_wxyz_relative = T_left_target_wxyz_reduced.copy()
                    T_left_target_position_relative = T_left_target_position_reduced.copy()
                
                    
                else:
                    mink.move_mocap_to_frame(self.model, self.data, f"Left_gripper_center_target", "Left_gripper_center", "site")
                    T_left_to_upper_model_world = mink.SE3.from_rotation_and_translation(
                        rotation=mink.SO3(T_left_target_wxyz_relative),
                        translation=T_left_target_position_relative
                    )
                    
                if not self.is_right_relative_control:
                    T_right_target_position_reduced, T_right_target_wxyz_reduced = world_frame_to_reduced_world_frame(
                        self.model, self.data, T_right_target_position, T_right_target_wxyz
                    )
                    ...
                    T_right_target_wxyz_relative = T_right_target_wxyz_reduced.copy()
                    T_right_target_position_relative = T_right_target_position_reduced.copy()
                else:
                    mink.move_mocap_to_frame(self.model, self.data, f"Right_gripper_center_target", "Right_gripper_center", "site")
                    T_right_to_upper_model_world = mink.SE3.from_rotation_and_translation(
                        rotation=mink.SO3(T_right_target_wxyz_relative),
                        translation=T_right_target_position_relative
                    )
