...
def main():
    ...
    # actuator_ids = np.array([m.actuator(name).id for name in whole_body_dof_names])
    # ---- mink IK初始化 ----
    ...
    damp_cost = np.zeros((upper_model.nv))
    damp_cost[0] = 100
    tasks = [
        posture_task := mink.PostureTask(upper_model, cost=pose_cost, lm_damping=1.0),
        damping_task := mink.DampingTask(upper_model, damp_cost)
    ]
    ...
    max_torque = np.array([ 88,         # 你的 XML 里 motor forcerange 如有不同请自己改
                        25, 25, 25, 25,  25,  25,  25,
                        25, 25, 25, 25,  25,  25,  25          
                          ])
    pos_pid_controller = create_homie_biarm_pid_controllers()
    mink_init = False
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < config['simulation_duration']:
            ...
            arm_tau = compute_homie_control_torque(
                m,
                d,
                upper_dof_ids,
                pos_pid_controller,
